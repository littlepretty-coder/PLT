...
w_wo_t0 = data[,c(1,13,16,25,26)]
w_wo_t0 = merge(w_wo_t0,tax[,6:7],by.x = 'key',by.y = 'Species')
w_wo_t0 = w_wo_t0[w_wo_t0$Genus %in% names,]
mapping <- list(
  bac_1 = bac_1,
  bac_2 = bac_2,
  bac_3 = bac_3,
...
for (name in names(mapping)) {
  w_wo_t0$Module[w_wo_t0$Genus %in% mapping[[name]]] <- name
}
w_wo_t0 <- w_wo_t0 %>%
...
w_wo_t0 = w_wo_t0[rowSums(w_wo_t0[,2:3])>0,]
w_wo_t0$fdr <- p.adjust(w_wo_t0$p_w_wo_t3, method = "fdr")
#拉普拉斯变换 最小非零值的1/10 最小非零值为0.75 这里取0.1
min <- min(unlist(w_wo_t0[,2:3][w_wo_t0[,2:3] != 0]))
w_wo_t0$log2fc = log2((w_wo_t0$median_wt3+min)/(w_wo_t0$median_wot3+min))
data <- add_regulate(w_wo_t0, log2FC_name = "log2fc",
                     fdr_name = "p_w_wo_t3",log2FC = 1, fdr = 0.01)
