...
w_t23 = data[,c(1,12,13,19,26)]


w_t23 = merge(w_t23,tax[,6:7],by.x = 'key',by.y = 'Species')
w_t23 = w_t23[w_t23$Genus %in% names,]
mapping <- list(
  bac_1 = bac_1,
  bac_2 = bac_2,
  bac_3 = bac_3,
...
for (name in names(mapping)) {
  w_t23$Module[w_t23$Genus %in% mapping[[name]]] <- name
}
w_t23 <- w_t23 %>%
...
w_t23 = w_t23[rowSums(w_t23[,2:3])>0,]
w_t23$fdr <- p.adjust(w_t23$p_wt23, method = "fdr")
#拉普拉斯变换 最小非零值的1/10 最小非零值为0.75 这里取0.1
min <- min(unlist(w_t23[,2:3][w_t23[,2:3] != 0]))
w_t23$log2fc = log2((w_t23$median_wt3+min)/(w_t23$median_wt2+min))
data <- add_regulate(w_t23, log2FC_name = "log2fc",
                     fdr_name = "p_wt23",log2FC = 1, fdr = 0.01)
