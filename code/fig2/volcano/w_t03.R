...
# wo_t13= data0[,c(1,10,14,22)]
w_t03 = data[,c(1,10,13,26,28)]

w_t03 = merge(w_t03,tax[,6:7],by.x = 'key',by.y = 'Species')
w_t03 = w_t03[w_t03$Genus %in% names,]
mapping <- list(
  bac_1 = bac_1,
  bac_2 = bac_2,
  bac_3 = bac_3,
...
for (name in names(mapping)) {
  w_t03$Module[w_t03$Genus %in% mapping[[name]]] <- name
}
w_t03 <- w_t03 %>%
...
w_t03 = w_t03[rowSums(w_t03[,2:3])>0,]
w_t03$fdr <- p.adjust(w_t03$p_wt03, method = "fdr")
#拉普拉斯变换 最小非零值的1/10 最小非零值为0.75 这里取0.1
min <- min(unlist(w_t03[,2:3][w_t03[,2:3] != 0]))
w_t03$log10_median_wt0 = log10(w_t03$median_wt0+min)
w_t03$log10_median_wt3 = log10(w_t03$median_wt3+min)
w_t03$log2fc = w_t03$log10_median_wt3/w_t03$log10_median_wt0
data <- add_regulate(w_t03, log2FC_name = "log2fc",
...
# 绘制火山图并添加标注
data[data[, 14] == "Not Significant", 8] <- "grey"
# 绘制火山图并添加标注
