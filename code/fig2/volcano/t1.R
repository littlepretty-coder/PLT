...
w_wo_t1 = data[,c(1,11,15,23,26)]

w_wo_t1 = merge(w_wo_t1,tax[,6:7],by.x = 'key',by.y = 'Species')
w_wo_t1 = w_wo_t1[w_wo_t1$Genus %in% names,]
mapping <- list(
  bac_1 = bac_1,
  bac_2 = bac_2,
  bac_3 = bac_3,
...
for (name in names(mapping)) {
  w_wo_t1$Module[w_wo_t1$Genus %in% mapping[[name]]] <- name
}
w_wo_t1 <- w_wo_t1 %>%
...
w_wo_t1 = w_wo_t1[rowSums(w_wo_t1[,2:3])>0,]
w_wo_t1$fdr <- p.adjust(w_wo_t1$p_w_wo_t1, method = "fdr")
#拉普拉斯变换 最小非零值的1/10 最小非零值为0.75 这里取0.1
min <- min(unlist(w_wo_t1[,2:3][w_wo_t1[,2:3] != 0]))
w_wo_t1$log2fc = log2((w_wo_t1$median_wt1+min)/(w_wo_t1$median_wot1+min))
data <- add_regulate(w_wo_t1, log2FC_name = "log2fc",
                     fdr_name = "p_w_wo_t1",log2FC = 1, fdr = 0.01)
