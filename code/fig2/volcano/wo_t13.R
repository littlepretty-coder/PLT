...
# wo_t13= data0[,c(1,10,14,22)]
wo_t13 = data[,c(1,15,16,21,26)]



wo_t13 = merge(wo_t13,tax[,6:7],by.x = 'key',by.y = 'Species')
wo_t13 = wo_t13[wo_t13$Genus %in% names,]
mapping <- list(
  bac_1 = bac_1,
  bac_2 = bac_2,
  bac_3 = bac_3,
...
for (name in names(mapping)) {
  wo_t13$Module[wo_t13$Genus %in% mapping[[name]]] <- name
}
wo_t13 <- wo_t13 %>%
...
wo_t13 = wo_t13[rowSums(wo_t13[,2:3])>0,]
wo_t13$fdr <- p.adjust(wo_t13$p_wot13, method = "fdr")
#拉普拉斯变换 最小非零值的1/10 最小非零值为0.75 这里取0.1
min <- min(unlist(wo_t13[,2:3][wo_t13[,2:3] != 0]))
wo_t13$log2fc = log2((wo_t13$median_wot3+min)/(wo_t13$median_wot1+min))
data <- add_regulate(wo_t13, log2FC_name = "log2fc",
                     fdr_name = "p_wot13",log2FC = 1, fdr = 0.01)
