...
w_t12 = data[,c(1,11,12,18,26)]


w_t12 = merge(w_t12,tax[,6:7],by.x = 'key',by.y = 'Species')
w_t12 = w_t12[w_t12$Genus %in% names,]
mapping <- list(
  bac_1 = bac_1,
  bac_2 = bac_2,
  bac_3 = bac_3,
...
for (name in names(mapping)) {
  w_t12$Module[w_t12$Genus %in% mapping[[name]]] <- name
}
w_t12 <- w_t12 %>%
...
w_t12 = w_t12[rowSums(w_t12[,2:3])>0,]
w_t12$fdr <- p.adjust(w_t12$p_wt12, method = "fdr")
#拉普拉斯变换 最小非零值的1/10 最小非零值为0.75 这里取0.1
min <- min(unlist(w_t12[,2:3][w_t12[,2:3] != 0]))
w_t12$log2fc = log2((w_t12$median_wt2+min)/(w_t12$median_wt1+min))
data <- add_regulate(w_t12, log2FC_name = "log2fc",
                     fdr_name = "p_wt12",log2FC = 1, fdr = 0.01)
