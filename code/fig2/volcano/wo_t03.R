...
# wo_t03= data0[,c(1,10,14,22)]
# wo_t03= data0[,c(1,10,14,22)]
# wo_t03= data0[,c(1,10,14,22)]
# wo_t01= data0[,c(1,10,14,22)]
# wo_t03= data0[,c(1,10,14,22)]
wo_t03 = data[,c(1,14,16,27,28)]



wo_t03 = merge(wo_t03,tax[,6:7],by.x = 'key',by.y = 'Species')
wo_t03 = wo_t03[wo_t03$Genus %in% names,]
mapping <- list(
  bac_1 = bac_1,
  bac_2 = bac_2,
  bac_3 = bac_3,
...
for (name in names(mapping)) {
  wo_t03$Module[wo_t03$Genus %in% mapping[[name]]] <- name
}
wo_t03 <- wo_t03 %>%
...
wo_t03 = wo_t03[rowSums(wo_t03[,2:3])>0,]
wo_t03$fdr <- p.adjust(wo_t03$p_wot03, method = "fdr")
#拉普拉斯变换 最小非零值的1/10 最小非零值为0.75 这里取0.1
min <- min(unlist(wo_t03[,2:3][wo_t03[,2:3] != 0]))
wo_t03$log2fc = log2((wo_t03$median_wot3+min)/(wo_t03$median_wot0+min))
data <- add_regulate(wo_t03, log2FC_name = "log2fc",
                     fdr_name = "p_wot03",log2FC = 1, fdr = 0.01)
