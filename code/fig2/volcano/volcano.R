...
w_wo_t0$fdr <- p.adjust(w_wo_t0$p_w_wo_t0, method = "fdr")
#拉普拉斯变换 最小非零值的1/10 最小非零值为0.75 这里取0.1
min <- min(unlist(w_wo_t0[,2:3][w_wo_t0[,2:3] != 0]))
w_wo_t0$log2fc = log2((w_wo_t0$median_wt0+min)/(w_wo_t0$median_wot0+min))
data <- add_regulate(w_wo_t0, log2FC_name = "log2fc",
                     fdr_name = "p_w_wo_t0",log2FC = 1, fdr = 0.01)
