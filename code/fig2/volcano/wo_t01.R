...
wo_t01 = data[,c(1,14,15,20,26)]





wo_t01 = merge(wo_t01,tax[,6:7],by.x = 'key',by.y = 'Species')
wo_t01 = wo_t01[wo_t01$Genus %in% names,]
mapping <- list(
  bac_1 = bac_1,
  bac_2 = bac_2,
  bac_3 = bac_3,
...
for (name in names(mapping)) {
  wo_t01$Module[wo_t01$Genus %in% mapping[[name]]] <- name
}
wo_t01 <- wo_t01 %>%
...
wo_t01 = wo_t01[rowSums(wo_t01[,2:3])>0,]
wo_t01$fdr <- p.adjust(wo_t01$p_wot01, method = "fdr")
#拉普拉斯变换 最小非零值的1/10 最小非零值为0.75 这里取0.1
min <- min(unlist(wo_t01[,2:3][wo_t01[,2:3] != 0]))
wo_t01$log2fc = log2((wo_t01$median_wot1+min)/(wo_t01$median_wot0+min))
data <- add_regulate(wo_t01, log2FC_name = "log2fc",
                     fdr_name = "p_wot01",log2FC = 1, fdr = 0.01)
